...
class IntentPlanEvidence(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["evidence_type", "field", "source_reference", "text"]

    @field_validator('evidence_type')
    def evidence_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['source_quote', 'product_signal', 'outcome_signal', 'rule']):
            raise ValueError("must be one of enum values ('source_quote', 'product_signal', 'outcome_signal', 'rule')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IntentPlanEvidence from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IntentPlanEvidence from a dict"""
        if obj is None:
            return None
